...
interface CobroModalProps {
  ...
  onClose: () => void
  onCobrar: (datos: { monto: number, observaciones: string, creditos: number, alumnosAPagar: string[], concepto: string }) => void
}

export default function CobroModal({ abierto, familia, modeloNegocio, onClose, onCobrar }: CobroModalProps) {
  const [alumnosAPagar, setAlumnosAPagar] = useState<string[]>(familia.map(f => f.id))
  ...
  const handleSubmit = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    onCobrar({ monto, observaciones, creditos, alumnosAPagar, concepto: conceptoSeleccionado })
  }
      ...
      <div className="bg-card border border-border shadow-2xl rounded-3xl w-full max-w-md overflow-hidden flex flex-col max-h-[90vh]">
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-5 overflow-y-auto custom-scrollbar">
          {familia.length > 1 && (
            <div className="space-y-2 border-b border-border pb-4">
              ...
              <div className="space-y-2 mt-2">
                {familia.map(fam => (
                  <div key={fam.id} onClick={() => toggleCheckAlumno(fam.id)} className={`p-3 border rounded-xl flex items-center justify-between cursor-pointer transition-colors ${alumnosAPagar.includes(fam.id) ? 'bg-primary/10 border-primary' : 'bg-background border-border opacity-60'}`}>
                    <div className="flex items-center gap-3">
                      ...
                      <span className="font-bold text-sm uppercase">{fam.nombre} {fam.apellido}</span>
                    </div>
                  </div>
                ))}
              </div>
          ...
          <div className="space-y-1">
            ...
            <Input name="observaciones" type="text" placeholder="Ej: Mes Abril" className="h-12 rounded-xl border-border bg-background" />
          </div>

          <div className="flex gap-2 pt-2">
            <Button type="button" onClick={onClose} variant="outline" className="flex-1 rounded-xl h-12 font-bold text-muted-foreground">Cancelar</Button>
